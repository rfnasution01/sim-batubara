...
import { useGetJenisPenghargaanFungsionalQuery } from '@/store/slices/referensiAPI'
...
export function SelectListNamaJabatan({
  name,
  headerLabel,
  placeholder,
  isDisabled,
  ...
  const idPegawai = localStorage.getItem('pegawaiID')
  const [query, setQuery] = useState<string>(null)
  const [riwayatJabatan, setRiwayatJabatan] = useState<JenisJabatanType[]>()
  ...
  useEffect(() => {
    if (!isFetching) {
      if (data?.meta?.page > 1) {
        setRiwayatJabatan((prevData) => [...prevData, ...(data?.data ?? [])])
      } else {
        setRiwayatJabatan([...(data?.data ?? [])])
      }
    }
  }, [data])

  let NamaJabatanOption = []
  if (isSuccess) {
    NamaJabatanOption = riwayatJabatan?.map((item) => {
      return {
        value: item?.id,
        label: item?.nama,
      }
            ...
            <div className="w-full phones:w-full">
              <FormControl>
                <Select
                  ...
                  className={'text-[2rem]'}
                  options={NamaJabatanOption}
                  value={
                    NamaJabatanOption?.filter(
